...
from tts.vocab import standardize_vocab_token
...
class _G2PWrapper(_G2P):
    def __init__(self, g2p: _G2P, specialized_vocab: Mapping[str, str]) -> None:
        ...
        self.specialized_vocab = {
            standardize_vocab_token(token): phonemes
    ...
    def __call__(self, text: str) -> tuple[str, list[MToken]]:
        ...
        for i, token in enumerate(tokens):
            if known_phonemes := self.specialized_vocab.get(standardize_vocab_token(token.text)):
